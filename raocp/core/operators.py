...
class Operator:
        ...
        for i in range(self.__num_nonleaf_nodes):
            ...
            for j in children_of_i:
                output_dual[self.__segment_d[3] + j] = \
                    sqrtm(self.__raocp.nonleaf_cost_at_node(j).nonleaf_state_weights) @ states[i]
            ...
            output_dual[self.__segment_d[7] + i] = self.__raocp.gamma

        for i in range(self.__num_nonleaf_nodes, self.__num_nodes):
            stage_at_i = self.__raocp.tree.stage_of(i)
            output_dual[self.__segment_d[11] + i] = \
                sqrtm(self.__raocp.leaf_cost_at_node(i).leaf_state_weights) @ states[i]
    ...
    def ell_transpose(self, input_dual, output_primal):
        ...
        for i in range(self.__num_nonleaf_nodes):
            ...
            for j in children_of_i:
                output_primal[self.__segment_p[1] + i] += \
                    sqrtm(self.__raocp.nonleaf_cost_at_node(j).nonleaf_state_weights) @ dual_3[j]
        ...
        for i in range(self.__num_nonleaf_nodes, self.__num_nodes):
            stage_at_i = self.__raocp.tree.stage_of(i)
            output_primal[self.__segment_p[1] + i] = \
                sqrtm(self.__raocp.leaf_cost_at_node(i).leaf_state_weights) @ dual_11[i]
